...
defmodule Resume.Certifications do
  ...
  the `count` closest certifications by cosine distance, will 
  return as `return_type`
  """
  @spec top_embeds(user :: map(), input_string :: String.t(), count :: integer(), :map | :json) ::
          {:ok, list(Certification.t())} | {:error, CertificationsError.t()}
  def top_embeds(user = %User{}, input_string, count, :map)
    ...
    with {:ok, embedding} <-
      ...
      q =
        from certification in Certification,
          where: certification.user_id == ^user.id,
          order_by: cosine_distance(certification.embedding, ^embedding),
          limit: ^count,
          select: %{
            description: certification.description,
            long_description: certification.embedding_content,
      ...
      {:error, e} ->
        {:error, %CertificationsError{reason: e}}
    end
  end
